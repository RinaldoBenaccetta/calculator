...
const createElement = (elementTag, elementClass, elementId) => {
    // create a new section element
    const newElement = document.createElement(elementTag);
    // add class to it
    newElement.classList.add(elementClass);
...
export const createInDom = (elementTag, elementClass, elementId, parentId) => {
    // get the parent element
    const parent = document.querySelector(`#${parentId}`);
    // create and append new element to parent
    parent.append(createElement(elementTag, elementClass, elementId));
};
